...
export async function before(m) {
	...
	if (chat && chat.updateAnime) {
		...
		setInterval(async () => {
			...
			if (/Movie/.test(detailAnime.episode)) return conn.reply(m.chat, 'වීඩියෝ ගොනුව විශාල වැඩි නිසා යැවිය නොහැක...', quoted)
			let res = await downloadAnime(zippy?.['480p'] || zippy?.['720p'] || zippy?.['360p']).catch(() => null)
			if (!res) return conn.reply(m.chat, '.බාගත කිරීමේ සබැඳිය තවමත් නොමැත..', quoted)
